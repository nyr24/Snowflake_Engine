...
void input_process_key(Key key, bool is_pressed) {
    if (state.kb_curr.keys[static_cast<u8>(key)] != is_pressed) {
        ...
        event_fire(static_cast<u8>(is_pressed ? SystemEventCode::KEY_PRESSED : SystemEventCode::KEY_RELEASED), nullptr, &context);
    }
}
...
void input_process_button(MouseButton button, bool is_pressed) {
...
void input_process_mouse_move(MousePos pos) {
    if (pos != state.mouse_curr.pos) {
        ...
        event_fire(static_cast<u8>(SystemEventCode::MOUSE_MOVED), nullptr, &context);
    }
}
...
void input_process_mouse_wheel(i8 z_delta) {
    ...
    event_fire(static_cast<u8>(SystemEventCode::MOUSE_WHEEL), nullptr, &context);
}
