...
concept AllocatorTrait = requires(A a, Args... args) {
    ...
    { a.reallocate(std::declval<T*>(), std::declval<u32>(), std::declval<u16>()) } -> std::same_as<ReallocReturn>;
    { a.reallocate_handle(std::declval<usize>(), std::declval<u32>(), std::declval<u16>()) } -> std::same_as<ReallocReturnHandle>;
